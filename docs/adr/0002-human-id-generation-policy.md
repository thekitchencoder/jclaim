# ADR-0002: Human-ID generation as a policy — pluggable generator port with composable acceptance filtering
...
`humanId` is the opt-in, human-facing display ID minted alongside an entity's
URN at registration (`DefaultEntityResolver.freshHumanId()`). It is intended to
...
Today `HumanIdGenerator` is a concrete `final` class. It draws entropy from a
`Supplier<Long>`, masks it through a `HumanIdFormat`, and renders the result as
...
ADR-0001). Promoting human-ID generation to the same shape is consistent with
the existing grain, not a new pattern.

## Decision

**1. Promote `HumanIdGenerator` to a minimal port.** A single-method
...
```java
@FunctionalInterface
public interface HumanIdGenerator {
...
**3. Rename the current class to `CrockfordHumanIdGenerator`.** It is, precisely,
a Crockford *generation* policy: random entropy → `HumanIdFormat` → Crockford +
Damm. An Open Location Code generator (`OlcHumanIdGenerator`) is its eventual
**sibling** — both exist, neither replaces the other.

**4. Apply acceptance by composition, not by forking.** A
`FilteringHumanIdGenerator` decorator wraps any delegate `HumanIdGenerator` plus
a predicate and re-rolls the delegate, within a bounded attempt budget, until the
predicate accepts. Because the decorator is *itself* a `HumanIdGenerator`, it
...
`FilteringHumanIdGenerator(new CrockfordHumanIdGenerator(...), allowAll)`. With
...
on the concrete `CrockfordHumanIdGenerator`, off the port. The filter runs at
...
| Blast radius | `jclaim-core` id + resolver wiring; breaks `new HumanIdGenerator(...)` callers |
...
### Option C: One combined `HumanIdPolicy` doing both generation and acceptance
...
**Pros:** No new type; `freshHumanId()` just checks a predicate before the
...
part of the contract *at all*: a decorator that is itself a `HumanIdGenerator`
...
- Adding generation policies — OLC-20 (vowel-resistant), sequential, UUID/ULID,
  externally supplied — each as a small `HumanIdGenerator`.
...
- Per-deployment **and per-type** tolerance: multi-type mode already gives each
  `jclaim.entity-types.<type>` its own `human-id.template`; a filter/generator
  slot drops in the same way (`entity-types.person.human-id.filter: strict`,
  `entity-types.invoice.human-id.filter: off`), and per-type physical isolation
...
- Turning a concrete `final class` into an interface breaks anyone calling
  `new HumanIdGenerator(...)`. With `0.1.0` and `0.2.0` already on Maven Central
...
**Neutral**
- `humanId` stays opt-in: a `null` generator still means "mint no human ID."
- Uniqueness, alias atomicity, and stewardship-event behaviour are untouched. The
  resolver's `freshHumanId()` loop continues to own *only* the uniqueness re-roll;
...
public final class CrockfordHumanIdGenerator implements HumanIdGenerator {
...
// Acceptance applied by composition. Itself a HumanIdGenerator, so it wraps
// any generation policy. Allow-all (the default) makes it a transparent no-op.
public final class FilteringHumanIdGenerator implements HumanIdGenerator {

    public static final Predicate<String> ALLOW_ALL = s -> true;

    private final HumanIdGenerator delegate;
    private final Predicate<String> acceptable;
    private final int maxAttempts;

    public FilteringHumanIdGenerator(HumanIdGenerator delegate,
    ...
    public String generate() {
        ...
        throw new IllegalStateException(
                "No acceptable human ID after " + maxAttempts + " attempts");
    }
}
```

Default wiring (template configured): the resolver builder composes
`new FilteringHumanIdGenerator(new CrockfordHumanIdGenerator(format, entropy), ALLOW_ALL, n)`.
Blank/absent template still yields a `null` generator and no human ID.

## Action Items

**This change**
1. [ ] Extract `HumanIdGenerator` as `@FunctionalInterface { String generate(); }`.
2. [ ] Rename the concrete class to `CrockfordHumanIdGenerator`; keep
   `isValid`/`format()` on it, off the port.
3. [ ] Add `FilteringHumanIdGenerator(delegate, Predicate<String>, maxAttempts)`
   with an `ALLOW_ALL` default predicate.
4. [ ] Wire `DefaultEntityResolver.Builder` (and the Spring starter's template
   path) to compose `Filtering(Crockford, ALLOW_ALL)`; preserve null-means-no-id.
5. [ ] Confirm `freshHumanId()` still owns only the uniqueness loop; the
   acceptance loop lives in the decorator.
6. [ ] Fix tests that construct the old class; add decorator tests — allow-all
...
7. [ ] `OlcHumanIdGenerator` (vowel-resistant 20-symbol alphabet) + Damm-radix
...
9. [ ] Per-type config keys
   (`entity-types.<type>.human-id.{filter,generator}`).
...
specialised per deployment, with no change forced on `EntityStorage` or its
conformance contract. The structural background — Crockford's single-letter
